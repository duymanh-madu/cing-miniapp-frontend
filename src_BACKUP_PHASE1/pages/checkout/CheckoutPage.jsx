...
import {
  usePaymentStore,
} from "@/payment/store/paymentStore";
...
function CheckoutPage() {
  ...
  const memberActivated =
    useRuntimeCustomerIdentityStore(
      (state) =>
        state.memberActivated
    );

  const setPaymentSession =
    usePaymentStore(
      (state) =>
        state.setPaymentSession
    );

  const setPaymentError =
    usePaymentStore(
      (state) =>
        state.setPaymentError
    );
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      if (
        ...
        alert(
          "Vui lòng kích hoạt hội viên trước khi thanh toán"
        );
      ...
      /**
       * ===============================================
       * RUNTIME CUSTOMER VALIDATION
       * ===============================================
       */

      const customerPhone =
        runtimeIdentity?.phone ||
        null;

      const customerName =
        runtimeIdentity?.fullName ||
        "Zalo Member";

      const customerId =
        runtimeIdentity?.customerId ||
        null;
