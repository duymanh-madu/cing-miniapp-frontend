...
export default function SnakeGame({ profile, onExit }) {
  ...
  useEffect(() => {
    ...
    sock.on("connect", () => {
      sock.emit("game:rooms");
    });

    sock.on("game:rooms", (data) => setRooms(data));

    sock.on("game:joined", ({ roomId, rooms: r }) => {
      setRooms(r || []);
      setPhase("playing");
    ...
    sock.on("game:state", (state) => {
      ...
      if (state.self) {
        ...
        const effs = Object.keys(state.self.effects || {});
        setEffects(effs);
      }
    });
  ...
  useEffect(() => {
    ...
    const handleMove = (cx, cy) => {
      ...
      const mx = cx - rect.left - W/2;
      const my = cy - rect.top  - H/2;
    ...
    const onTouchStart = (e) => {
      ...
      if (e.touches.length >= 2) {
        ...
        socketRef.current?.emit("game:boost", { active: true });
      }
    };
    const onTouchEnd = () => {
      inputRef.current.boosting = false;
      socketRef.current?.emit("game:boost", { active: false });
    };
    ...
    function drawPearl(cx, cy, r, glowAmt, glowColor) {
      if (glowAmt > 0.05) {
        const g = ctx.createRadialGradient(cx,cy,0,cx,cy,r*2.5);
        g.addColorStop(0, `${glowColor}${Math.floor(glowAmt*150).toString(16).padStart(2,"0")}`);
      ...
      const g2 = ctx.createRadialGradient(cx-r*0.18,cy-r*0.18,0,cx,cy,r*0.44);
      ...
      ctx.beginPath(); ctx.ellipse(cx-r*0.18,cy-r*0.2,r*0.22,r*0.16,-0.3,0,Math.PI*2);
      ctx.fillStyle="rgba(255,180,100,0.75)"; ctx.fill();
    }

    function drawSnake(player, isSelf, camX, camY, glow) {
      const segs = player.segments;
      if (!segs || segs.length === 0) return;
      const maxR = Math.min(7 + player.length * 0.18, 20);
      const minR = maxR * 0.3;
      const glowColor = isSelf ? "#FF6010" : "#8040FF";

      // Glow thân
      if (glow > 0.05 && isSelf) {
        ctx.save();
        ctx.filter = `blur(${4+glow*4}px)`;
        ctx.strokeStyle = `rgba(255,96,16,${glow*0.5})`;
        ctx.lineWidth = maxR*2.4; ctx.lineCap="round"; ctx.lineJoin="round";
        ctx.beginPath();
        segs.forEach((s,i) => {
      ...
      for (let i = segs.length-1; i >= 0; i--) {
        ...
        drawPearl(sx,sy,r, isSelf?glow:0, glowColor);
      }
      ...
      if (segs.length > 0) {
        ...
        const hr = maxR*1.6;
        const angle = segs.length>1 ? Math.atan2(segs[0].y-segs[1].y, segs[0].x-segs[1].x) : 0;
        drawHead(hx,hy,hr,angle,isSelf,glow,player);
      }
    }

    function drawHead(x,y,r,angle,isSelf,glow,player) {
      ctx.save(); ctx.translate(x,y); ctx.rotate(angle);
      // Glow
      if (glow>0.05&&isSelf) {
        const g=ctx.createRadialGradient(0,0,0,0,0,r*2);
        g.addColorStop(0,`rgba(255,96,16,${glow*0.7})`);
        g.addColorStop(1,"rgba(0,0,0,0)");
        ctx.beginPath(); ctx.arc(0,0,r*2,0,Math.PI*2); ctx.fillStyle=g; ctx.fill();
      }
      // Base
      ...
      [[-r*0.28,-r*0.26],[r*0.28,-r*0.26]].forEach(([ex,ey],idx)=>{
        ...
        ctx.beginPath(); ctx.arc(ex,ey+er*0.08,er*0.78,0,Math.PI*2); ctx.fillStyle="#160800"; ctx.fill();
        const ig=ctx.createRadialGradient(ex-er*0.2,ey,0,ex,ey,er*0.46);
        ig.addColorStop(0,"#E85010"); ig.addColorStop(1,"#600800");
        ctx.beginPath(); ctx.arc(ex,ey+er*0.1,er*0.42,0,Math.PI*2); ctx.fillStyle=ig; ctx.fill();
        ctx.beginPath(); ctx.arc(ex,ey+er*0.12,er*0.25,0,Math.PI*2); ctx.fillStyle="#050100"; ctx.fill();
        ctx.beginPath(); ctx.ellipse(ex+er*0.2,ey-er*0.26,er*0.2,er*0.15,-0.3,0,Math.PI*2);
        ctx.fillStyle="rgba(255,255,255,0.95)"; ctx.fill();
      });
      // Tongue
      ctx.strokeStyle="#EE0808"; ctx.lineWidth=2.2; ctx.lineCap="round";
      ctx.beginPath(); ctx.moveTo(0,r*0.5); ctx.lineTo(0,r*0.72); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0,r*0.72); ctx.lineTo(-r*0.16,r*0.9); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0,r*0.72); ctx.lineTo(r*0.16,r*0.9); ctx.stroke();
      // Name tag
      if (!isSelf) {
        ctx.fillStyle="rgba(0,0,0,0.6)"; ctx.beginPath();
        ctx.roundRect(-r*1.2,-r*1.8,r*2.4,r*0.7,4); ctx.fill();
        ctx.fillStyle="white"; ctx.font=`bold ${r*0.42}px sans-serif`;
        ctx.textAlign="center"; ctx.fillText(player.name||"",0,-r*1.28);
      }
      ctx.restore();
    }
    ...
    function drawFood(food, camX, camY) {
      food.forEach(f => {
        ...
        const r = f.size || 5;
        ctx.beginPath(); ctx.arc(fx,fy,r*1.4,0,Math.PI*2);
        const g=ctx.createRadialGradient(fx,fy,0,fx,fy,r*1.4);
        g.addColorStop(0,f.color||"#D86020"); g.addColorStop(1,"rgba(0,0,0,0)");
        ctx.fillStyle=g; ctx.fill();
        ...
        ctx.beginPath(); ctx.ellipse(fx-r*0.2,fy-r*0.2,r*0.25,r*0.18,-0.3,0,Math.PI*2);
        ctx.fillStyle="rgba(255,180,100,0.7)"; ctx.fill();
      });
    }

    function drawSpecial(items, camX, camY) {
      const ICONS = { x2:"✖2", x5:"✖5", x10:"✖10", magnet:"🧲", shield:"🛡" };
      const COLORS = { x2:"#4080FF", x5:"#C040FF", x10:"#FFD700", magnet:"#40FFB0", shield:"#FFB830" };
      items.forEach(s => {
        const sx = s.x-camX+W/2, sy = s.y-camY+H/2;
        ...
        g.addColorStop(0,`${COLORS[s.type]}88`); g.addColorStop(1,"rgba(0,0,0,0)");
        ctx.beginPath(); ctx.arc(sx,sy,r*2,0,Math.PI*2); ctx.fillStyle=g; ctx.fill();
        // Circle
        ctx.beginPath(); ctx.arc(sx,sy,r,0,Math.PI*2);
        ctx.fillStyle="#1a0d05"; ctx.fill();
        ctx.strokeStyle=COLORS[s.type]; ctx.lineWidth=2; ctx.stroke();
        // Icon
        ctx.font=`bold ${r*0.9}px sans-serif`; ctx.textAlign="center"; ctx.textBaseline="middle";
        ctx.fillStyle=COLORS[s.type]; ctx.fillText(ICONS[s.type]||s.type,sx,sy);
      });
    }

    function drawGrid(camX, camY) {
      ctx.strokeStyle="rgba(255,100,20,0.06)"; ctx.lineWidth=0.5;
      const gs=80, ox=(-camX%gs+W/2)%gs, oy=(-camY%gs+H/2)%gs;
    ...
    function frame() {
      ...
      if (localGlow>0) localGlow-=0.008;
  ...
  const EFFECT_COLORS = { x2:"#4080FF", x5:"#C040FF", x10:"#FFD700", magnet:"#40FFB0", shield:"#FFB830" };
  ...
  if (phase === "lobby") return (
    <div style={{ background:"#080504", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ textAlign:"center", marginBottom:28 }}>
        ...
        <p style={{ color:"#888", fontSize:12, margin:0 }}>Ăn trân châu · Tiêu diệt đối thủ · Leo rank</p>
      </div>

      {error && (
        <div style={{ background:"rgba(244,67,54,0.15)", border:"1px solid #f44336",
      ...
      <div style={{ width:"100%", maxWidth:320, marginBottom:20 }}>
        {rooms.map(r => (
          <div key={r.id} style={{ background:"#1a0d05", borderRadius:12, padding:"14px 18px",
            marginBottom:10, border:`1px solid ${r.full?"#444":"#D4531C44"}`,
            display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div>
              <p style={{ color:"white", fontWeight:800, margin:"0 0 3px" }}>Phòng {r.id}</p>
              <p style={{ color:"#888", fontSize:12, margin:0 }}>{r.players}/{r.max} người chơi</p>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <div style={{ display:"flex", gap:2 }}>
                {Array.from({length:5}).map((_,i)=>(
                  <div key={i} style={{ width:6, height:6, borderRadius:3,
                    background: i < Math.ceil(r.players/r.max*5) ? "#D4531C" : "#333" }}/>
                ))}
              </div>
      ...
      <div style={{ background:"#1a0d05", borderRadius:12, padding:"14px 18px",
        ...
        <p style={{ color:"#888", fontSize:11, fontWeight:700, margin:"0 0 10px", letterSpacing:1 }}>LUẬT CHƠI</p>
        <p style={{ color:"#aaa", fontSize:12, margin:"0 0 6px" }}>🫧 Ăn trân châu để to ra và mở rộng tầm nhìn</p>
        <p style={{ color:"#aaa", fontSize:12, margin:"0 0 6px" }}>⚡ Tạt đầu đối thủ để tiêu diệt họ → +100đ</p>
        <p style={{ color:"#aaa", fontSize:12, margin:"0 0 6px" }}>🗺 Rắn to = tầm nhìn rộng hơn</p>
        <p style={{ color:"#aaa", fontSize:12, margin:0 }}>👆 Chạm 2 ngón tay để tăng tốc</p>
      </div>

      <button onClick={joinRoom}
        style={{ background:"#D4531C", border:"none", color:"white",
          borderRadius:14, padding:"16px 48px", fontSize:16, fontWeight:900,
      ...
      <button onClick={onExit}
        style={{ background:"none", border:"none", color:"#666",
          marginTop:16, fontSize:13, cursor:"pointer" }}>
        ← Quay lại
      </button>
    </div>
  );
  ...
  if (phase === "dead") return (
    <div style={{ background:"#080504", minHeight:"100vh", display:"flex",
      ...
      <h2 style={{ color:"white", fontSize:22, fontWeight:900, margin:"0 0 8px" }}>Bạn đã bị tiêu diệt!</h2>
      {deathInfo?.killerName && (
        <p style={{ color:"#D4531C", fontSize:14, margin:"0 0 24px" }}>
          Bởi <strong>{deathInfo.killerName}</strong>
        </p>
      )}

      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:28, width:"100%", maxWidth:280 }}>
        {[
          { label:"Số kill", value: deathInfo?.kills || 0, icon:"⚔️", color:"#D4531C" },
          { label:"Điểm", value: (deathInfo?.kills||0)*100, icon:"⭐", color:"#FFD700" },
          { label:"Độ dài", value: deathInfo?.length || 0, icon:"📏", color:"#4CAF50" },
          { label:"Hạng", value: `#${deathInfo?.rank||"?"}`, icon:"🏆", color:"#9C27B0" },
        ].map((s,i) => (
          <div key={i} style={{ background:"#1a0d05", borderRadius:12, padding:"16px",
      ...
      <button onClick={() => { setPhase("lobby"); setDeathInfo(null); setKills(0); setMyLength(0); }}
        style={{ background:"#D4531C", border:"none", color:"white",
          borderRadius:14, padding:"14px 40px", fontSize:15, fontWeight:900,
          cursor:"pointer", marginBottom:12, width:"100%", maxWidth:280 }}>
        🔄 Chơi lại
      </button>
      <button onClick={onExit}
        style={{ background:"none", border:"1px solid #333", color:"#888",
          borderRadius:14, padding:"12px 40px", fontSize:14, cursor:"pointer",
  ...
  return (
    <div style={{ position:"relative", width:"100vw", height:"100vh", overflow:"hidden", background:"#080504" }}>
      ...
      <div style={{ position:"absolute", top:10, left:10, right:10, display:"flex", justifyContent:"space-between", pointerEvents:"none" }}>
        ...
        <div style={{ background:"rgba(0,0,0,0.6)", borderRadius:10, padding:"8px 14px", textAlign:"right" }}>
          ...
          <div style={{ display:"flex", gap:4, justifyContent:"flex-end" }}>
            {effects.map(e => (
              <span key={e} style={{ background:EFFECT_COLORS[e]+"33", color:EFFECT_COLORS[e],
                borderRadius:4, padding:"1px 5px", fontSize:10, fontWeight:800 }}>
                {EFFECT_ICONS[e]}
              </span>
            ))}
          </div>
      ...
      <div style={{ position:"absolute", top:65, right:10, background:"rgba(0,0,0,0.65)",
        borderRadius:10, padding:"8px 12px", minWidth:130, pointerEvents:"none" }}>
        <p style={{ color:"#D4531C", fontSize:10, fontWeight:800, margin:"0 0 6px", letterSpacing:1 }}>TOP KILLS</p>
