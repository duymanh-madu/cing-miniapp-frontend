...
export default function AdminMembers({ token }) {
  ...
  const [action, setAction] = useState(null); // "block" | "chat_lock"
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const until = duration === "forever" ? null : new Date(Date.now() + parseDuration(duration)).toISOString();
      setMembers(ms => ms.map(m => m.user_id === selected.user_id ? {
        ...m,
        is_blocked: action === "block" ? true : m.is_blocked,
        chat_locked_until: action === "chat_lock" ? until : m.chat_locked_until,
      } : m));
      setMsg(`Đã ${action === "block" ? "khoá tài khoản" : "khoá chat"} ${selected.zalo_name}`);
  ...
  const handleUnblock = async (m) => {
    ...
    try {
      ...
      setMembers(ms => ms.map(p => p.user_id === m.user_id ? { ...p, is_blocked: false, chat_locked_until: null } : p));
      setMsg("Đã mở khoá " + m.zalo_name);
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      setShowAdd(false); setNewPhone(""); setNewName("");
      setTimeout(() => setMsg(""), 2000);
    } catch(e) { setMsg("Lỗi: " + e.message); }
    setSaving(false);
  };

  const parseDuration = (d) => ({ "1d":86400000, "3d":259200000, "7d":604800000, "30d":2592000000, "forever":999*86400000 }[d] || 604800000);

  const DURATIONS = [
    { key:"1d", label:"1 ngày" }, { key:"3d", label:"3 ngày" },
    { key:"7d", label:"7 ngày" }, { key:"30d", label:"30 ngày" }, { key:"forever", label:"Vĩnh viễn" },
  ];
      ...
      {showAdd && (
        <div style={{ background:"#1a1a24", borderRadius:12, padding:16, marginBottom:16, border:"1px solid #333" }}>
          ...
          <div style={{ display:"flex", gap:10, marginBottom:10 }}>
            <input value={newPhone} onChange={e => setNewPhone(e.target.value)} placeholder="Số điện thoại *"
              style={{ flex:1, background:"#0f0f13", border:"1px solid #333", borderRadius:8, padding:"8px 12px", color:"white", fontSize:13 }}/>
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Tên (tuỳ chọn)"
              style={{ flex:1, background:"#0f0f13", border:"1px solid #333", borderRadius:8, padding:"8px 12px", color:"white", fontSize:13 }}/>
          </div>
          <button onClick={handleAddMember} disabled={saving}
            style={{ background:"#D4531C", border:"none", borderRadius:8, padding:"8px 20px", color:"white", fontSize:13, fontWeight:700, cursor:"pointer" }}>
            Thêm
          </button>
        </div>
      )}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#1a1a24", borderRadius:16, padding:24, width:360, border:"1px solid #333" }}>
            <p style={{ fontSize:16, fontWeight:800, margin:"0 0 8px" }}>
              {action === "block" ? "🔒 Khoá tài khoản" : "🔇 Khoá chat"}
            </p>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={handleAction} disabled={saving}
                style={{ flex:1, background:"#D4531C", border:"none", borderRadius:8, padding:"10px", color:"white", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                Xác nhận
              </button>
              <button onClick={() => { setAction(null); setSelected(null); }}
                style={{ flex:1, background:"#2a2a38", border:"none", borderRadius:8, padding:"10px", color:"white", fontSize:13, cursor:"pointer" }}>
                Huỷ
              </button>
            </div>
          </div>
          ...
          {filtered.slice(0,100).map(m => {
            ...
            return (
              <div key={m.user_id} style={{ background:"#1a1a24", borderRadius:10, border:"1px solid #2a2a38", padding:"12px 16px" }}>
                ...
                <span style={{ fontSize:11, fontWeight:700, color: chatLocked ? "#ff9800" : "#4CAF50" }}>
                  {chatLocked ? "🔇 Khoá" : "✓ OK"}
                </span>
                <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                  {(isBlocked || chatLocked) ? (
                    <button onClick={() => handleUnblock(m)} disabled={saving}
                      style={{ padding:"4px 8px", borderRadius:6, border:"1px solid #4CAF50", background:"transparent", color:"#4CAF50", fontSize:10, cursor:"pointer" }}>
                      Mở khoá
                    </button>
                  ) : (
                    <>
                      <button onClick={() => { setSelected(m); setAction("block"); }}
                        style={{ padding:"4px 8px", borderRadius:6, border:"1px solid #f44336", background:"transparent", color:"#f44336", fontSize:10, cursor:"pointer" }}>
                        Khoá TK
                      </button>
                      <button onClick={() => { setSelected(m); setAction("chat_lock"); }}
                        style={{ padding:"4px 8px", borderRadius:6, border:"1px solid #ff9800", background:"transparent", color:"#ff9800", fontSize:10, cursor:"pointer" }}>
                        Khoá chat
                      </button>
                    </>
                  )}
